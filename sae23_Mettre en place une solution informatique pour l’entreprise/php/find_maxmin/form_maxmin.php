...
<head>                   <!--La variable "head" c'est pour reperer l'entete du site, on y met tout les bases du site sans qu'il ne sois concretement visible sur le site (sauf pour le title et le favicon)-->
	<title>Max/Min Data</title>                                                        <!-- Variable titre -->
...
<body>                                    <!-- A partir de cette variable jusqu'a sa fermeture, il y'aura tout le contenu du site-->
  ...
  <header>      <!--En tete-->
        <h1> Trouve la longitude (Max & Min) et la latitude (Max & Min) en fonction du groupe <br> <span class="redfamily">(tappez le groupe souhaité)</span></h1>
    </header>
  ...
  <input type="checkbox" id="check_menu" >       <!-- La balise imput a la fonctionnalité de créer un contrôle interactif dans un formulaire web qui permet à l'utilisateur de saisir des données, dans mon cas sous forme de box -->
	...
	<div class="nav-list">
        ...
        <a class="micon" href="../../mode_emploie_search23" target="_blank"><i class="fa fa-user-circle-o"></i><span>Help</span></a>
...
<div class="search-wrapper"><!-- Dans ce div nous allon crée la barre de recherche -->
	<form method="post" action="form_maxmin.php">   <!-- Avec la methode POST le formulaire va chercher les fonction dans le fichier form_maxmin.php -->
...
<!-- Fin en-tête -->
  ...
  printf ("<tbody>");
    printf ("<tr>");
        ...
        printf ("<td><strong>%s</strong></td>", $r["longitude_max"]);
        printf ("<td><strong>%s</strong></td>", $r["longitude_min"]);
        printf ("<td><strong>%s</strong></td>", $r["latitude_max"]);
        printf ("<td><strong>%s</strong></td>", $r["latitude_min"]);
    printf ("</tr>");
  printf ("</tbody>");
  printf ("</table>");
